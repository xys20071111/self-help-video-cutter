...
export async function getFileList(ctx: Context) {
	...
	for (const dir of config.recordDir) {
		...
		for await (const file of files) {
			if (
				...
				if (status.code === 0) {
					const ffprobeOutputRaw = (
						await ffprobeProgress.stdout.getReader().read()
					).value
					const ffprobeOutputRawJSON =
						decoder.decode(ffprobeOutputRaw)
	...
	response.body = {
		code: 0,
		files: fileInfo.sort((a, b) => b.birthtime - a.birthtime),
	}
}
