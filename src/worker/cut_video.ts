...
const M3U8_HEADER = new Uint8Array([
    35, 69, 88, 84, 77, 51, 85, 10, 35, 69, 88, 84,
    45, 88, 45, 86, 69, 82, 83, 73, 79, 78, 58, 55,
    10, 35, 69, 88, 84, 45, 88, 45, 84, 65, 82, 71,
    69, 84, 68, 85, 82, 65, 84, 73, 79, 78, 58, 49,
    10, 35, 69, 88, 84, 45, 88, 45, 83, 84, 65, 82,
    84, 58, 84, 73, 77, 69, 45, 79, 70, 70, 83, 69,
    84, 61, 48, 10
])
...
function splitTime(time: string): number {
    if (time.includes(':')) {
        ...
        if (times.length === 3) {
            return 60 * 60 * parseInt(times[0]) + 60 * parseInt(times[1]) + parseInt(times[2])
        } else {
...
setInterval(async () => {
    ...
    if (msg.input.endsWith('m3u8')) {
        ...
        const targetClips = playlist.clips.slice(startClipIndex, stopClipIndex + 1)
        const tmpFile = await Deno.makeTempFile({
            suffix: '.m3u8'
        })
        await Deno.writeFile(tmpFile, M3U8_HEADER)
        await Deno.writeFile(tmpFile, encoder.encode(`#EXT-X-MAP:URI="${playlist.mapFile}"\n`), { append: true })
