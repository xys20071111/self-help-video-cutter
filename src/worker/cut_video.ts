...
setInterval(async () => {
	...
	const msg: ITask = taskQueue[0];
	taskQueue.splice(0, 1);
	await client.set(["clip", msg.uuid], msg);
	...
	if (msg.input.endsWith("m3u8")) {
		...
		const startTime = splitTime(msg.start);
		const stopTime = splitTime(msg.end);
		...
		if (taskStatus.code === 0) {
			await client.set(["clip", msg.uuid], { ...msg, status: 1 });
		} else {
			postMessage(decoder.decode(taskStatus.stderr));
			await client.set(["clip", msg.uuid], { ...msg, status: 2 });
		}
	} else {
